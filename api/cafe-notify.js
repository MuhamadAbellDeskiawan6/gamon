...
export default async function handler(req, res) {
    if (req.method === 'GET') {
        ...
        return res.redirect(302, `/cafe-success.html?orderId=${orderId}`);
    }
    ...
    if (req.method === 'POST') {
        ...
        if (orderId && (status === 'SUCCESS' || status === 'PAID')) {
            try {
                const db = getDb();
                await db.collection('cafe_vouchers').doc(orderId).update({ 
                    status: "PAID",
                    redeemCode: `CAFE-${orderId}-${Math.floor(1000 + Math.random() * 9000)}`,
                    updatedAt: new Date() 
                });
                console.log(`Voucher Kafe ${orderId} berhasil diupdate ke PAID`);
                return res.status(200).send("OK");
            } catch (e) {
                console.error("Gagal update Firestore Kafe:", e);
