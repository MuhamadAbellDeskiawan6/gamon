...
export default async function handler(req, res) {
    ...
    try {
        const { amount, orderId, airName, voucherName, customerName, customerPhone } = req.body;
        ...
        if (!existingQuery.empty) {
            ...
            return res.status(200).json({
                response: {
                    payment: {
                        url: `/air-success.html?orderId=${existingData.orderId}`
                    }
                }
            });
        }
        ...
        const redeemCode = 'AIR-' + Math.random().toString(36).substring(2, 8).toUpperCase();

        const requestBody = {
            order: {
                amount: Number(amount),
                invoice_number: orderId,
                callback_url: `${baseUrl}/air-success.html?orderId=${orderId}`
            },
            payment: {
                payment_due_date: 60,
                return_url: `${baseUrl}/air-success.html?orderId=${orderId}`,
        ...
        if (dukuResponse.status === 200 && data.response?.payment?.url) {
            try {
                await db.collection('air_vouchers').doc(orderId).set({
                    orderId,
                    redeemCode,
                    amount: Number(amount),
                    airName: airName || "Air Partner",
